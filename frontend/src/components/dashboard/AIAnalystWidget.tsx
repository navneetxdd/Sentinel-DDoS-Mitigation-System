...
export const AIAnalystWidget = ({ telemetry }: AIAnalystWidgetProps) => {
  const [analysis, setAnalysis] = useState<string>("Monitoring network traffic...");
  const [isAnalyzing, setIsAnalyzing] = useState(false);
  const [lastAnalyzedScore, setLastAnalyzedScore] = useState(0);
  ...
  useEffect(() => {
    ...
    const runAnalysis = async () => {
      ...
      setIsAnalyzing(true);
      setAnalysis("Analyzing current traffic patterns...");
      
      try {
        const result = await analyzeThreat(telemetry);
        setAnalysis(result);
        setLastAnalyzedScore(telemetry.threatScore);
      } catch (err) {
        setAnalysis("Analysis failed. See console for details.");
      } finally {
        setIsAnalyzing(false);
      }
      
      // Schedule next analysis in 20 seconds
